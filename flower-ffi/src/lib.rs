...
/// Starts flower with options, on a successful start this function blocks the current
/// thread.
///
/// @note This is not a stable API, parameters will change from time to time.
///
/// @param rt_id A unique ID to associate this flower instance, this is required when
...
#[no_mangle]
pub extern "C" fn flower_run_with_options(
    rt_id: u16,
    ...
    if let Ok(config_path) = unsafe { CStr::from_ptr(config_path).to_str() } {
        if let Err(e) = flower::util::run_with_options(
            rt_id,
...
/// Starts flower with a single-threaded runtime, on a successful start this function
...
/// @return ERR_OK on finish running, any other errors means a startup failure.
#[no_mangle]
pub extern "C" fn flower_run(rt_id: u16, config_path: *const c_char) -> i32 {
    if let Ok(config_path) = unsafe { CStr::from_ptr(config_path).to_str() } {
        let opts = flower::StartOptions {
            config: flower::Config::File(config_path.to_string()),
            #[cfg(feature = "auto-reload")]
            auto_reload: false,
...
#[no_mangle]
pub extern "C" fn flower_reload(rt_id: u16) -> i32 {
    if let Err(e) = flower::reload(rt_id) {
        return to_errno(e);
    }
    ERR_OK
}
...
#[no_mangle]
pub extern "C" fn flower_test_config(config_path: *const c_char) -> i32 {
    if let Ok(config_path) = unsafe { CStr::from_ptr(config_path).to_str() } {
        if let Err(e) = flower::test_config(&config_path) {
            return to_errno(e);
        }
        ERR_OK
    } else {
