...
pub fn from_lines(lines: Vec<io::Result<String>>) -> Result<Config> {
    ...
    for line in general_lines {
        ...
        match parts[0] {
            ...
            "socks-listen" => {
                ...
                general.socks_port = get_value::<u16>(port);
            }
            "api-interface" => {
    ...
    for line in rule_lines {
        ...
        match rule.type_field.as_str() {
            ...
            // "RULE-SET" => {
            //     let url  = params[1].clone().to_string();
            //     let get_url_future = async move {
            ...
            //         .build()
            //         .unwrap();
            //     let body = rt.block_on(get_url_future);
            //     let lines = body.split('\n');
            //     for line in lines {
            //         let set_params = if let Some(p) = get_char_sep_slice(&line, ',') {
            //             p
            //         } else {
            //             continue;
            //         };
            ...
            //             continue; // at lease 2 params
            //         }
            //         let mut set_rule = Rule {
            //             type_field: set_params[0].to_string(),
            //             ..Default::default()
            //         };
            //         if set_params.len() != 2 {
            //             continue;
            //         }
            //         set_rule.target = params[2].to_string();
            //         match set_rule.type_field.as_str() {
            //             "IP-CIDR" | "DOMAIN" | "DOMAIN-SUFFIX" | "DOMAIN-KEYWORD" | "GEOIP" | "EXTERNAL"
            //             | "PORT-RANGE" | "NETWORK" | "INBOUND-TAG" | "PROCESS" => {
            //                 set_rule.filter = Some(set_params[1].to_string());
            //                 rules.push(set_rule);
            //             }
            //             _ => {}
            //         }
    ...
    if let Some(ext_rules) = conf.rule.as_mut() {
        for ext_rule in ext_rules.iter_mut() {
            ...
            match ext_rule.type_field.as_str() {
                ...
                "PROCESS" => {
                    rule.processes.push(ext_filter);
                }
                _ => {}
            }
            rules.push(rule);
