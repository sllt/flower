...
fn main() {
    ...
    if args.test {
        if let Err(e) = flower::test_config(&args.config) {
            println!("{:?}", e);
            exit(1);
        } else {
            println!("ok");
    ...
    if let Some(tag) = args.test_outbound {
        let config = flower::config::from_file(&args.config).unwrap();
        let rt = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .unwrap();
        rt.block_on(flower::util::test_outbound(&tag, &config));
        exit(0);
    }
